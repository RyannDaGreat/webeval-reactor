...
import { CustomProvider, Container, Content, Button, ButtonGroup } from 'rsuite';
...
function Image({ path, ...imgProps }: { path: string; [key: string]: any }) {
    ...
    return <img src={url} alt={path} {...imgProps} loading="lazy" />;
}
...
function FileBrowser() {
    ...
    useEffect(() => {
        const doFetch = async () => {
            const response = await webeval.exeval('json_scandir(p)', { p: currentPath });
            setEntries(response);
        };
        doFetch();
    }, [currentPath]);
    ...
    const goToInputPath = () => {
        const output = prompt('Enter the new Path', currentPath);
    ...
    return (
        <CustomProvider theme="dark">
            <Container className="app-container">
                <Content>
                    <PanelGroup direction="horizontal" autoSaveId="filebrowser-panels">
                        <Panel defaultSize={30} minSize={15}>
                            ...
                            <ButtonGroup vertical block>
                                ...
                                {entries.map((entry, index) => (
                                    <Button
                                        key={index}
                                        onClick={() => handleItemClick(entry.name, entry.isDirectory)}
                                        block
                                    >
                        ...
                        <PanelResizeHandle style={{ width: 6, background: '#444', cursor: 'col-resize' }} />
                        <Panel>
                            {selectedFile && (
                                <>
                                    <h3 style={{ padding: 10 }}>{selectedFile}</h3>
